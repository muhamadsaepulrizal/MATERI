...
class OrderApp(App):
    def build(self):
        ...
        self.menu_layout = BoxLayout(orientation='vertical', padding=20, spacing=20, size_hint=(0.8, 0.8))

        self.title_label = Label(text="Lakukan Layanan!", font_size='20sp', size_hint=(1, 0.1))
        self.menu_layout.add_widget(self.title_label)

        self.layanan_button = Button(text="Lakukan Pelayanan", size_hint=(1, 0.2), on_press=self.order_service)
        self.menu_layout.add_widget(self.layanan_button)

        self.list_button = Button(text="List Pesanan", size_hint=(1, 0.2), on_press=self.show_orders)
        self.menu_layout.add_widget(self.list_button)

        self.history_button = Button(text="Riwayat Pelanggan", size_hint=(1, 0.2), on_press=self.show_history)
    ...
    def order_service(self, instance):
        ...
        self.add_button = Button(text="Tambah ke antrian", size_hint=(1, 0.2), on_press=self.add_order)
        self.service_layout.add_widget(self.add_button)

        self.menu_button = Button(text="<- Menu", size_hint=(1, 0.2), on_press=self.main_menu)
    ...
    def show_orders(self, instance):
        ...
        self.call_button = Button(text="Panggil Pelanggan", size_hint=(0.5, 1), on_press=self.call_customer)
        self.menu_button = Button(text="<- Menu", size_hint=(0.5, 1), on_press=self.main_menu)
